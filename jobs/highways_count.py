...
spark: SparkSession = (
    SparkSession.builder.appName("highways_count")
    .config(
        "spark.jars.packages",
...
df = df.select(F.approx_count_distinct("highway").alias("count"))

def foreach_batch_function(df, epoch_id):
    data = df.toPandas().to_dict(orient="records")[0]
    connection["analysis"]["highways_count"].replace_one({"_id": 1}, data, upsert=True)
